...
export default function NewProformaPage() {
  ...
  const handleSaveProforma = async () => {
    ...
    try {
      ...
      if (party?.number && waStatusData?.status === 'connected' && waSettings?.auto_send_proforma) {
        const { data: storeData } = await supabase.from('store').select('*').limit(1).maybeSingle();

        const invoicePayload: InvoiceData = {
          type:            'proforma',
          template:        storeData?.proforma_template || storeData?.invoice_template || 'prestige',
          store:           storeData,
          bill_number:     pBillNo,
          date,
          ...
          items:           validItems.map(item => ({
            ...
            value:       (Number(item.qty) || 1) * Math.max(0, (Number(item.rate) || 0) - (Number(item.discount) || 0)),
          })),
          subtotal,
          discount:        Number(discount) || 0,
          trade_in_credit: totalTradeInCredit,
          ...
          due:             finalTotal,
          trade_ins:       tradeIns
            .filter(ti => ti.description.trim())
            .map(ti => ({ description: ti.description, qty: Number(ti.qty) || 1, rate: Number(ti.rate) || 0 })),
        };

        sendInvoiceMutation.mutate(
          { invoiceData: invoicePayload, phone: party.number, messageTemplate: waSettings.proforma_message_template },
          {
            onSuccess: () => success('Sent via WhatsApp', `Quotation sent to ${party.number}`),
      ...
      // Fire-and-forget: generate and store PDF for the new proforma
      triggerPdfGeneration(pData.id, 'proforma').catch(() => {});
