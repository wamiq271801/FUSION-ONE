...
export default function NewSalePage() {
  ...
  const handleSaveSale = async () => {
    ...
    try {
      ...
      // ── WhatsApp Auto-Send (fire-and-forget) ──────────────────────────────
      const party = parties.find(p => p.id === partyId);
      if (party?.number && waStatusData?.status === 'connected' && waSettings?.auto_send_sale) {
        const mappedInvItems = selectedItems.map(item => ({
          brand: item.brand, model: item.model, imei: item.imei,
          ram_rom: item.ram_rom, color: item.color,
          qty: 1,
          rate:     Number(item.base_selling_price) || Number(item.sold_price),
          price:    Number(item.base_selling_price) || Number(item.sold_price),
          discount: Math.max(0, (Number(item.base_selling_price) || 0) - (Number(item.sold_price) || 0)),
          value:    Number(item.sold_price),
        }));

        const { data: storeData } = await supabase.from('store').select('*').limit(1).maybeSingle();

        const invoicePayload: InvoiceData = {
          type:              'sale',
          template:          storeData?.invoice_template || 'prestige',
          store:             storeData,
          bill_number:       saleBillNo,
          date,
          party:             { name: party.name, number: party.number },
          items:             mappedInvItems,
          subtotal,
          discount:          Number(discount) || 0,
          trade_in_credit:   totalTradeInCredit,
          final_total:       finalTotal,
          paid:              nPaid,
          due:               Math.max(0, finalTotal - nPaid),
          trade_ins:         tradeIns.map(ti => ({ ...ti, credit_value: Number(ti.credit_value), mrp: Number(ti.mrp) || undefined })),
        };

        sendInvoiceMutation.mutate(
          { invoiceData: invoicePayload, phone: party.number, messageTemplate: waSettings.sale_message_template },
          {
            onSuccess: () => success('Sent via WhatsApp', `Invoice sent to ${party.number}`),
            onError:   (err: any) => { const e = resolveWaError(err); error(e.title, e.message); },
          },
        );
      }
      // ─────────────────────────────────────────────────────────────────────
      ...
      // Fire-and-forget: generate and store PDF for the new sale
      triggerPdfGeneration(saleData.id, 'sale').catch(() => {});
