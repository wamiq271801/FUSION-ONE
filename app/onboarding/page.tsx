  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      if (logoFile) {
        ...
        const { data, error: uploadErr } = await supabase.storage.from('store_assets').upload(filePath, logoFile);
        if (!uploadErr && data) {
      ...
      // We use sequential inserts. In a real environment, if tables don't exist yet, this throws.
      // But we fulfill the Phase 4 logic.
      
      const { data: storeData, error: storeErr } = await supabase.from('store').insert({
      ...
      const { data: bankData, error: bankErr } = await supabase.from('bank_accounts').insert([
      ...
      if (mainBank && paymentModes.length > 0) {
        const modesToInsert = paymentModes.map(m => ({ bank_account_id: mainBank.id, name: m }));
        await supabase.from('payment_modes').insert(modesToInsert);
      }

      const { data: fyData, error: fyErr } = await supabase.from('financial_years').insert({
        start_date: fyStart,
        end_date: fyEnd,
        status: 'active',
        sale_counter: 0,
      ...
      const { error: updErr } = await supabase.from('store')
    ...
    <div className="flex items-center justify-center mb-8 gap-3">
      {[1, 2, 3].map((num) => (
        <React.Fragment key={num}>
          <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-colors ${step === num ? 'bg-indigo-600 text-white shadow-md shadow-indigo-600/30' : step > num ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100 text-slate-400'}`}>
            {step > num ? <CheckCircle2 className="w-5 h-5" /> : num}
          </div>
          {num < 3 && <div className={`w-12 h-1 rounded-full ${step > num ? 'bg-indigo-200' : 'bg-slate-100'}`} />}
        </React.Fragment>
      ))}
    </div>
  );
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <Card className="w-full max-w-2xl bg-white shadow-xl shadow-slate-200/50 border-slate-200 p-2 sm:p-4">
        <CardHeader className="text-center">
          <CardTitle className="text-2xl sm:text-3xl font-bold text-slate-900 tracking-tight">Business Setup</CardTitle>
        ...
        <CardContent>
          ...
          <div className="min-h-[300px]">
              ...
              <div className="space-y-6 animate-in slide-in-from-right-4 fade-in duration-300">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1 md:col-span-2">
                    ...
                    <Input placeholder="Fusion Store" value={storeName} onChange={(e) => setStoreName(e.target.value)} />
                  </div>
                  <div className="space-y-1 md:col-span-2">
                  ...
                  <div className="space-y-1">
                    ...
                    <Input type="email" placeholder="contact@store.com" value={email} onChange={(e) => setEmail(e.target.value)} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-sm font-medium text-slate-700">Website</label>
                    <Input placeholder="www.store.com" value={website} onChange={(e) => setWebsite(e.target.value)} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-sm font-medium text-slate-700">GSTIN</label>
                    <Input placeholder="Optional Tax ID" value={gstin} onChange={(e) => setGstin(e.target.value)} />
                  </div>
                  <div className="space-y-1 md:col-span-2">
            ...
            {step === 2 && (
              <div className="space-y-6 animate-in slide-in-from-right-4 fade-in duration-300">
                ...
                <p className="text-sm text-slate-600 bg-slate-50 p-3 rounded-md border border-slate-200">
                  A <strong>Cash</strong> account will be created automatically with no payment modes. 
                  Below, define your primary bank account and its incoming/outgoing payment modes.
                </p>

                <div className="space-y-4">
                  <div className="space-y-1">
                    <label className="text-sm font-medium text-slate-700">First Bank Account Name *</label>
                    <Input placeholder="e.g. HDFC Current Account" value={bankName} onChange={(e) => setBankName(e.target.value)} />
                  </div>
                  
                  <div className="space-y-2 pt-2">
                    <label className="text-sm font-medium text-slate-700">Supported Payment Modes</label>
                    <div className="flex gap-2">
                      ...
                      />
                      <Button variant="secondary" onClick={handleAddMode} type="button">Add</Button>
                    </div>
                    
                    <div className="flex flex-wrap gap-2 mt-4">
                      {paymentModes.map((mode) => (
                        <div key={mode} className="inline-flex items-center gap-1.5 bg-indigo-50 text-indigo-700 px-3 py-1.5 rounded-full text-sm font-medium border border-indigo-100">
                          {mode}
                          <button onClick={() => removeMode(mode)} className="hover:text-indigo-900 transition-colors">
                            <X className="w-3.5 h-3.5" />
                          </button>
                        </div>
                      ))}
                      {paymentModes.length === 0 && <span className="text-sm text-slate-400 italic">No modes added</span>}
                    </div>
                  </div>
                </div>
              </div>
            ...
            {step === 3 && (
              <div className="space-y-6 animate-in slide-in-from-right-4 fade-in duration-300">
                ...
                <p className="text-sm text-slate-600 bg-slate-50 p-3 rounded-md border border-slate-200">
                  Set up your first financial year. All transactions and stock will be recorded under this ledger period. You can create the next year when this one ends.
                </p>
          ...
          <div className="flex justify-between items-center mt-8 pt-4 border-t border-slate-100">
            ...
            {step < 3 ? (
              <Button onClick={nextStep} className="min-w-[100px]">Next</Button>
            ) : (
              <Button onClick={handleFinish} isLoading={isLoading} className="min-w-[120px]">Finish Setup</Button>
            )}
          </div>
        </CardContent>
      </Card>
